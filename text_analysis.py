...
# =============================================
# 【功能说明】
#   将问卷中的开放式文本（Q17情感词、Q22改进建议、各题的"其他"内容）
#   批量发送给大模型 API，获取情感、主题、摘要、建议价值等标签。
#
# 【核心策略】
#   1. 一次性全量调用：将所有文本合并为一条消息，只调用 1 次 API
#      → 避免多次调用的预热开销，速度提升 10 倍以上
#   2. 逐条分析：每条反馈独立成行（【条目 N】），避免合并解读
#   3. 语境增强：在用户消息中插入游戏黑话/梗的语境说明
#      → 让模型主动调用内部知识，而非死记词典
#   4. 后处理兜底：短文本自动提升建议价值，含实体词的主题重新归类
#
# 【输入】  清洗后问卷数据.xlsx
# 【输出】  文本分析结果.xlsx（47 条文本 + 情感/主题/摘要/建议价值标签）
# 【依赖】  .env 文件需配置 LLM_API_KEY、LLM_BASE_URL、LLM_MODEL
# =============================================
...
# 1. 配置大模型客户端
# ------------------------------
# 从 .env 读取配置，支持 OpenAI 及兼容 API（如 DeepSeek、智谱等）
...
# 3. 提取需要分析的文本
# ------------------------------
# 排除 19_其他内容（游戏名称列），因其为客观实体名称，无需情感分析
# 保留 10_其他内容（挫折/不满中的其他内容）、14_其他内容、15_其他内容等
...
# 4. 构建合并文本（每条反馈独立成行）
# ------------------------------
# 每条反馈独立成【条目 N】，确保大模型逐条分析，而非合并同用户的多条反馈
text_parts = []
...
# ------------------------------
# 提示词设计思路：
#   - 系统提示词：定义角色（游戏社区分析师）+ 核心原则 + 推理步骤
#   - 语境增强：在用户消息中列举常见梗（海猫、善、牢等），激活模型内部知识
#   - 输出格式：强制 JSON 数组，便于结构化解析
#   - 短文本保护：明确要求 1-5 字短文本不能默认 low
...
# 语境增强：在用户消息中补充游戏黑话/梗的上下文，帮助模型理解
CONTEXT_ENHANCEMENT = """
📌 分析语境：
- 这些反馈来自《明日方舟》玩家的问卷
...
# 8.5 后处理修正（通用兜底）
# ------------------------------
# 无需硬编码词条列表，基于文本长度和内容特征自动修正：
#   1. 短文本（≤5字符）+ low → 提升为 medium
#   2. 短文本 + neutral + 含中文且无负面词 → 倾向 positive
#   3. 主题为"其他"+ 含游戏实体词 → 重归类为"游戏内容"
